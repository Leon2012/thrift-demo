...
type RpcService interface {
  ...
  //  - FuncCode
  //  - ParamMap
  FunCall(callTime int64, funcCode string, paramMap map[string]string) (r []string, err error)
}
...
//  - FuncCode
//  - ParamMap
func (p *RpcServiceClient) FunCall(callTime int64, funcCode string, paramMap map[string]string) (r []string, err error) {
  if err = p.sendFunCall(callTime, funcCode, paramMap); err != nil { return }
  return p.recvFunCall()
}

func (p *RpcServiceClient) sendFunCall(callTime int64, funcCode string, paramMap map[string]string)(err error) {
...
var retval []string
  var err2 error
  if retval, err2 = p.handler.FunCall(args.CallTime, args.FuncCode, args.ParamMap); err2 != nil {
...
type FunCallArgs struct {
  ...
  FuncCode string `thrift:"funcCode,2" json:"funcCode"`
...
func (p *FunCallArgs) GetFuncCode() string {
return p.FuncCode
}
...
func (p *FunCallArgs) writeField2(oprot thrift.TProtocol) (err error) {
  if err := oprot.WriteFieldBegin("funcCode", thrift.STRING, 2); err != nil {
    return fmt.Errorf("%T write field begin error 2:funcCode: %s", p, err); }
  if err := oprot.WriteString(string(p.FuncCode)); err != nil {
  return fmt.Errorf("%T.funcCode (2) field write error: %s", p, err) }
  if err := oprot.WriteFieldEnd(); err != nil {
    return fmt.Errorf("%T write field end error 2:funcCode: %s", p, err); }
  return err
}
